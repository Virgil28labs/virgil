...
export function useChat(
  ...
  const { complete, completeStream, loading, error, clearError } =
    useLLM(config);
  ...
  const addMessage = useCallback(
    (role: "user" | "assistant" | "system", content: string): ChatMessage => {
      const message = {
  ...
  const regenerateLastResponse = useCallback(
    async (options: Partial<LLMRequest> = {}): Promise<ChatMessage | null> => {
  ...
  const exportConversation = useCallback(
    (format: ExportFormat = "json"): string | ExportData => {
      const exportData = {
        systemPrompt,
