...
import type { Habit, UserHabitsData, Achievement } from "../types/habit.types";
...
export const useHabits = () => {
  ...
  const checkIn = useCallback((habitId: string) => {
    setUserData((prev) => {
      ...
      const allCompleted = updatedHabits.every((h) => h.lastCheckIn === today);

      const perfectDays =
        allCompleted && !prev.stats.perfectDays.includes(today)
    ...
    return achievements.map((achievement) => {
      ...
      switch (achievement.requirement.type) {
        case "total-checkins": {
          ...
          progress = Math.min(
            100,
            (totalCheckIns / achievement.requirement.value) * 100,
          );
        ...
        case "streak": {
          ...
          progress = Math.min(
            100,
            (maxStreak / achievement.requirement.value) * 100,
          );
        ...
        case "perfect-week": {
          ...
          progress = Math.min(
            100,
            (recentPerfectDays / achievement.requirement.value) * 100,
          );
        ...
        case "all-habits":
          progress = Math.min(
            100,
            (habits.length / achievement.requirement.value) * 100,
          );
  ...
  const updateHabit = useCallback(
    (habitId: string, updates: { name?: string; emoji?: string }) => {
      setUserData((prev) => ({
        ...prev,
  ...
  const bestStreakStartDate = useCallback(() => {
    ...
    const habitWithBestStreak = userData.habits.reduce(
      (best, habit) =>
