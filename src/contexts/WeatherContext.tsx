...
export function WeatherProvider({ children }: WeatherProviderProps) {
  ...
  const fetchWeather = useCallback(
    async (forceRefresh: boolean = false): Promise<void> => {
      ...
      const isCacheValid =
        state.lastUpdated && Date.now() - state.lastUpdated < cacheExpiry;
      ...
      if (!coordinates && !ipLocation?.city) {
        ...
        try {
          const fallbackWeatherData =
            await weatherService.getWeatherByCoordinates(37.7749, -122.4194);
          ...
          try {
            const fallbackForecastData =
              await weatherService.getForecastByCoordinates(37.7749, -122.4194);
            dispatch({
              type: "SET_FORECAST_DATA",
              payload: fallbackForecastData,
            });
          } catch (forecastError) {
          ...
          dispatch({
            type: "SET_ERROR",
            payload: "Weather service unavailable",
          });
          return;
        }
      }
      ...
      try {
        ...
        dispatch({
          type: "SET_ERROR",
          payload: error.message || "Failed to fetch weather",
        });
      }
    },
  ...
  useEffect(() => {
    ...
    state.lastUpdated,
    fetchWeather,
  ]); // Boolean dependencies to prevent unnecessary effect runs
  ...
  const displayData = useMemo((): WeatherData | null => {
    ...
    if (state.unit === "celsius") {
      return {
        ...state.data,
        temperature: weatherService.convertTemperature(
          state.data.temperature,
          true,
        ),
        feelsLike: weatherService.convertTemperature(
