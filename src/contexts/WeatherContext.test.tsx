...
describe("WeatherContext", () => {
  ...
  it("provides initial state", async () => {
    ...
    const { result } = renderHook(() => useWeather(), {
      wrapper: ({ children }) =>
        wrapper({ children, locationValue: noLocationContext }),
    });
  ...
  it("fetches weather when location is available", async () => {
    mockWeatherService.getWeatherByCoordinates.mockResolvedValue(
      mockWeatherData,
    );
  ...
  it("updates weather when location changes", async () => {
    ...
    expect(
      mockWeatherService.getWeatherByCoordinates.mock.calls.length,
    ).toBeGreaterThan(1);
  });
