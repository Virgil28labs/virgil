...
describe('SearchService', () => {
    ...
    results: [
      {
        ...
        content: 'This is the first test result with some content that is quite long and should be truncated when displayed'
      },
      {
        title: 'Test Result 2',
  ...
  describe('search', () => {
    ...
    it('should handle HTTP error with error message', async () => {
      ...
      await expect(searchService.search(searchRequest)).rejects.toThrow(errorMessage)
    })
    ...
    it('should handle HTTP error without error message', async () => {
      ...
      await expect(searchService.search(searchRequest)).rejects.toThrow('Search service error: 500')
    })
    ...
    it('should handle unsuccessful response', async () => {
      ...
      await expect(searchService.search(searchRequest)).rejects.toThrow('Search engine unavailable')
    })
    ...
    it('should handle network errors', async () => {
      ...
      await expect(searchService.search(searchRequest)).rejects.toThrow('Network error')
    })
  })
  ...
  describe('checkHealth', () => {
    ...
    it('should return true when service is healthy', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://test-api.com/api/v1/search/health')
    })
    ...
    it('should return false when health check fails', async () => {
      ...
      expect(result).toBe(false)
      expect(consoleSpy).toHaveBeenCalledWith('Search health check failed:', expect.any(Error))
  ...
  describe('detectSearchIntent', () => {
    ...
    it('should be case insensitive', () => {
      expect(searchService.detectSearchIntent('SEARCH for something')).toBe(true)
    ...
    it('should detect triggers within sentences', () => {
      expect(searchService.detectSearchIntent('I want to search for recipes')).toBe(true)
      expect(searchService.detectSearchIntent('Can you find the nearest restaurant?')).toBe(true)
      expect(searchService.detectSearchIntent('Please tell me about quantum physics')).toBe(true)
    })
  })
  ...
  describe('extractSearchQuery', () => {
    it('should remove common prefixes', () => {
      ...
      expect(searchService.extractSearchQuery('lookup weather')).toBe('weather')
      expect(searchService.extractSearchQuery('tell me about React')).toBe('React')
      expect(searchService.extractSearchQuery('what is TypeScript')).toBe('TypeScript')
      expect(searchService.extractSearchQuery('research machine learning')).toBe('machine learning')
    })

    it('should remove question marks', () => {
      expect(searchService.extractSearchQuery('what is React?')).toBe('React')
      expect(searchService.extractSearchQuery('can you search for cats?')).toBe('cats')
    })

    it('should handle case insensitive prefixes', () => {
      expect(searchService.extractSearchQuery('Can You search for dogs')).toBe('dogs')
      expect(searchService.extractSearchQuery('PLEASE find cats')).toBe('cats')
      expect(searchService.extractSearchQuery('What Are cookies')).toBe('cookies')
    })
    ...
    it('should handle messages without prefixes', () => {
      expect(searchService.extractSearchQuery('quantum computing')).toBe('quantum computing')
      expect(searchService.extractSearchQuery('React hooks')).toBe('React hooks')
    })
  ...
  describe('formatSearchResults', () => {
    ...
    it('should truncate long content', () => {
      ...
      expect(formatted).toContain('This is the first test result with some content that is quite long and should be truncated when displayed...')
    ...
    it('should format results without answer', () => {
      ...
      const formatted = searchService.formatSearchResults(responseWithoutAnswer)
    ...
    it('should handle results without content', () => {
      const resultsWithoutContent: SearchResponse = {
        ...mockSearchResponse,
        results: [
          { title: 'No Content', url: 'https://example.com/nc' }
        ]
      }

      const formatted = searchService.formatSearchResults(resultsWithoutContent)
  ...
  describe('edge cases', () => {
    it('should handle very long queries', () => {
      ...
      const extracted = searchService.extractSearchQuery(`search for ${longQuery}`)
    ...
    it('should handle undefined in formatSearchResults', () => {
      const incompleteResponse = {
        ...
        query: 'test',
        results: [
          { title: undefined as any, url: 'https://example.com', content: null as any }
        ]
      } as SearchResponse
