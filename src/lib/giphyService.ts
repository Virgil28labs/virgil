...
const GIPHY_API_BASE =
  import.meta.env.VITE_GIPHY_API_URL || "https://api.giphy.com/v1";
...
class GiphyService {
  ...
  constructor() {
    if (!GIPHY_API_KEY || GIPHY_API_KEY === "your_giphy_api_key_here") {
      console.warn(
        "Giphy API key not configured. Please add VITE_GIPHY_API_KEY to your .env file",
      );
    }
  }
  ...
  private transformGiphyGif(gif: GiphyGif): GiphyImage {
    ...
    return {
      ...
      rating: gif.rating,
      width: parseInt(
        images.fixed_height?.width || images.downsized?.width || "0",
      ),
      height: parseInt(
        images.fixed_height?.height || images.downsized?.height || "0",
      ),
   ...
   */
  private async makeGiphyRequest(
    endpoint: string,
  ...
  ): Promise<{ gifs: GiphyImage[]; totalCount: number; hasMore: boolean }> {
  ...
  async getTrendingGifs(
  ...
  ): Promise<{ gifs: GiphyImage[]; hasMore: boolean }> {
    ...
    try {
      ...
      const gifs = data.data.map((gif) => this.transformGiphyGif(gif));
      const hasMore =
        gifs.length === (params.limit || this.defaultParams.limit);
