...
describe("weatherService", () => {
  ...
  describe("getWeatherByCoordinates", () => {
    ...
    it("handles API errors with JSON response", async () => {
      ...
      await expect(
        weatherService.getWeatherByCoordinates(40.7128, -74.006),
  ...
  describe("getWeatherByCity", () => {
    ...
    it("handles city API errors", async () => {
      ...
      await expect(
        weatherService.getWeatherByCity("Unknown City"),
      ).rejects.toThrow("Failed to fetch weather data");
    });
  });
  ...
  describe("getWeatherIconUrl", () => {
    it("returns correct icon URL", () => {
      expect(weatherService.getWeatherIconUrl("01d")).toBe(
        "https://openweathermap.org/img/wn/01d@2x.png",
      );

      expect(weatherService.getWeatherIconUrl("10n")).toBe(
  ...
  describe("getForecastByCoordinates", () => {
    ...
    it("fetches forecast data successfully", async () => {
      ...
      const result = await weatherService.getForecastByCoordinates(
        40.7128,
        -74.006,
      );
      ...
      expect(mockDedupeFetch).toHaveBeenCalledWith(
        "http://localhost:5002/api/v1/weather/forecast?lat=40.7128&lon=-74.006",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    it("uses cached forecast data when available", async () => {
      // First call to populate cache
      mockDedupeFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
      ...
      await weatherService.getForecastByCoordinates(40.7128, -74.006);
    ...
    it("refetches forecast when cache expires", async () => {
      // First call
      mockDedupeFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
      ...
      // Second call should refetch
      const updatedForecastData = { ...mockForecastData };
      updatedForecastData.list[0].temp = 75;
      mockDedupeFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({
      ...
      expect(result.list[0].temp).toBe(75);
      expect(mockDedupeFetch).toHaveBeenCalledTimes(2);
    ...
    it("handles forecast API errors", async () => {
      ...
      await expect(
        weatherService.getForecastByCoordinates(40.7128, -74.006),
      ).rejects.toThrow("Failed to fetch forecast data");
    });

    it("handles invalid forecast response structure", async () => {
      mockDedupeFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ success: false }),
  ...
  describe("getForecastByCity", () => {
    ...
    it("fetches forecast by city name", async () => {
      mockDedupeFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({
      ...
      const result = await weatherService.getForecastByCity("New York");

      expect(result).toEqual(mockForecastData);
      expect(mockDedupeFetch).toHaveBeenCalledWith(
        "http://localhost:5002/api/v1/weather/forecast?city=New+York",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    it("fetches forecast by city and country", async () => {
      mockDedupeFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({
      ...
      const result = await weatherService.getForecastByCity("New York", "US");

      expect(result).toEqual(mockForecastData);
      expect(mockDedupeFetch).toHaveBeenCalledWith(
        "http://localhost:5002/api/v1/weather/forecast?city=New+York&country=US",
        expect.any(Object),
      );
    });

    it("uses city forecast cache", async () => {
      mockDedupeFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({
      ...
      await weatherService.getForecastByCity("New York");
    ...
    it("handles city forecast API errors", async () => {
      mockDedupeFetch.mockResolvedValueOnce({
        ok: false,
        status: 404,
      } as Response);
      ...
      await expect(
        weatherService.getForecastByCity("Unknown City"),
      ).rejects.toThrow("Failed to fetch forecast data");
    });
  });
  ...
  describe("clearCache", () => {
    it("clears all cached data", async () => {
      ...
      expect(mockDedupeFetch).toHaveBeenCalledTimes(4);
    });
  });
});
