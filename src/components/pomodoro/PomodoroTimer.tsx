...
export const PomodoroTimer: React.FC<PomodoroTimerProps> = ({
  ...
  useEffect(() => {
    ...
    if (progress < 5) {
      ...
      setMotivationalMessage(
        `Halfway there, ${name}! Keep that momentum going 💪`,
      );
          ...
          >
            <svg width="280" height="280" className="clock-svg">
              ...
              {Array.from({ length: 12 }, (_, i) => {
                ...
                const x1 =
                  CENTER_X +
                  Math.cos((angle * Math.PI) / 180) * (CLOCK_RADIUS - 5);
                const y1 =
                  CENTER_Y +
                  Math.sin((angle * Math.PI) / 180) * (CLOCK_RADIUS - 5);
                const x2 =
                  CENTER_X +
                  Math.cos((angle * Math.PI) / 180) * (CLOCK_RADIUS - 10);
                const y2 =
                  CENTER_Y +
                  Math.sin((angle * Math.PI) / 180) * (CLOCK_RADIUS - 10);
              ...
              <path
                d={`M ${CENTER_X} ${CENTER_Y} L ${CENTER_X} ${CENTER_Y - INNER_RADIUS} A ${INNER_RADIUS} ${INNER_RADIUS} 0 ${
                  ...
                  CENTER_X +
                  Math.sin((fillAngle * Math.PI) / 180) * INNER_RADIUS
                } ${
                  CENTER_Y -
                  Math.cos((fillAngle * Math.PI) / 180) * INNER_RADIUS
                } Z`}
                fill="#6c3baa"
                opacity="0.9"
