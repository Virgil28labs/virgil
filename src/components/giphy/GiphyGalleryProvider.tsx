...
// Create the context
const GiphyGalleryContext = createContext<GiphyContextType | undefined>(
  undefined,
);
...
// Provider component
export function GiphyGalleryProvider({
  children,
  isOpen = false,
}: GiphyGalleryProviderProps) {
  ...
  const loadTrending = useCallback(async () => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to load trending GIFs";
      dispatch({ type: "SET_ERROR", payload: errorMessage });
    }
  }, [state.rating]);
...
export function useGiphyGallery(): GiphyContextType {
  ...
  if (context === undefined) {
    throw new Error(
      "useGiphyGallery must be used within a GiphyGalleryProvider",
    );
  }
  return context;
}
