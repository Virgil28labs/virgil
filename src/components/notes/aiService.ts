 ...
 */
export async function processEntryWithAI(
  content: string,
    ...
    try {
      ...
      if (attempt < AI_CONFIG.MAX_RETRIES - 1) {
        await new Promise((resolve) =>
          setTimeout(resolve, AI_CONFIG.RETRY_DELAY * (attempt + 1)),
        );
      }
    }
  }
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        mood:
          parsed.mood &&
          ["positive", "neutral", "negative"].includes(parsed.mood)
            ? parsed.mood
            : undefined,
      };
    } catch (parseError) {
