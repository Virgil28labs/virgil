...
describe('taskUtils', () => {
  describe('extractTasksFromContent', () => {
    it('should extract checkbox tasks from content', () => {
      ...
      expect(tasks[0]).toEqual({ text: 'Buy groceries', completed: false, extracted: false });
      expect(tasks[1]).toEqual({ text: 'Call mom', completed: true, extracted: false });
      expect(tasks[2]).toEqual({ text: 'Finish report', completed: false, extracted: false });
      expect(tasks[3]).toEqual({ text: 'Review code', completed: true, extracted: false });
    });
  ...
  describe('mergeTasksWithAI', () => {
    ...
    it('should merge without duplicates', () => {
      ...
      expect(merged.filter(t => t.text.toLowerCase().includes('milk'))).toHaveLength(1);
      expect(merged.some(t => t.text === 'Send email' && t.extracted)).toBe(true);
    });
