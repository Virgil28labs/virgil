...
describe("DogImageStates", () => {
  ...
  describe("Error State", () => {
    ...
    it("does not show error state when loading is true", () => {
      ...
      expect(
        screen.queryByText("Failed to fetch dogs"),
  ...
  describe("Empty State", () => {
    it("renders fetch tab empty state when activeTab is fetch", () => {
      render(
        <DogImageStates {...defaultProps} activeTab="fetch" dogsCount={0} />,
      );
    ...
    it("renders gallery tab empty state when activeTab is gallery", () => {
      render(
        <DogImageStates {...defaultProps} activeTab="gallery" dogsCount={0} />,
      );
    ...
    it("shows Go Fetch button in gallery empty state when onSwitchToFetch is provided", () => {
      ...
      expect(button).toHaveAttribute(
        "aria-label",
        "Switch to fetch tab to get dogs",
      );
    ...
    it("does not show empty state when there are dogs", () => {
      render(<DogImageStates {...defaultProps} dogsCount={5} />);

      expect(
        screen.queryByText("Ready to meet some doggos?"),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("Your Doggo Sanctuary is empty!"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe("No State (Dogs Present)", () => {
    it("returns null when there are dogs to display", () => {
      const { container } = render(
        <DogImageStates {...defaultProps} dogsCount={5} />,
      );
...
describe("DogImageSkeleton", () => {
  it("renders skeleton loading element", () => {
    ...
    expect(
      container.querySelector(".doggo-image-skeleton"),
    ).toBeInTheDocument();
  });
});
