...
describe("ImageModal", () => {
  ...
  describe("rendering", () => {
    ...
    it("should render null when currentIndex is null", () => {
      const { container } = render(
        <ImageModal {...defaultProps} currentIndex={null} />,
      );
    ...
    it("should render navigation buttons appropriately", () => {
      const { rerender } = render(
        <ImageModal {...defaultProps} currentIndex={1} />,
      );
    ...
    it("should show correct favorite button state", () => {
      ...
      // Favorited
      expect(screen.getByLabelText("Remove from favorites")).toHaveTextContent(
        "❤️",
      );
  ...
  describe("interactions", () => {
    ...
    it("should copy image URL", async () => {
      ...
      expect(mockCopyImageToClipboard).toHaveBeenCalledWith(
        "https://example.com/dog2.jpg",
      );
    });
  ...
  describe("keyboard navigation", () => {
    ...
    it("should not navigate past boundaries", () => {
      const { rerender } = render(
        <ImageModal {...defaultProps} currentIndex={0} />,
      );
  ...
  describe("edge cases", () => {
    it("should handle empty dogs array", () => {
      const { container } = render(
        <ImageModal {...defaultProps} dogs={[]} currentIndex={0} />,
      );
      expect(container.firstChild).toBeNull();
    });
    ...
    it("should handle out of bounds currentIndex", () => {
      const { container } = render(
        <ImageModal {...defaultProps} currentIndex={10} />,
      );
      expect(container.firstChild).toBeNull();
    });
    ...
    it("should handle single dog array", () => {
      render(
        <ImageModal {...defaultProps} dogs={[mockDogs[0]]} currentIndex={0} />,
      );
