    ...
    <div data-testid="dog-grid">
      {dogs.map((dog: DogImage, _index: number) => (
        <div key={dog.id} data-testid={`dog-${dog.id}`}>
          <button onClick={() => onImageClick(dog.url)}>
            View {dog.breed}
          </button>
...
const mockUseDogFavorites = useDogFavorites as jest.MockedFunction<
  typeof useDogFavorites
>;
...
describe("DogGallery", () => {
  ...
  describe("rendering", () => {
    ...
    it("should render tabs", () => {
      ...
      expect(
        screen.getByRole("tab", { name: /Fetch Doggos/i }),
      ).toBeInTheDocument();
      expect(
  ...
  describe("loading and error states", () => {
    it("should show loading state", () => {
      ...
      expect(
        screen.getByText("Fetching adorable doggos..."),
      ).toBeInTheDocument();
      expect(
        document.querySelector(".doggo-loading-spinner"),
      ).toBeInTheDocument();
    });
    ...
    it("should show empty state for fetch tab", () => {
      ...
      expect(
        screen.getByText("Ready to meet some doggos?"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Choose your preferences and click 'Fetch'"),
      ).toBeInTheDocument();
    });
    ...
    it("should show empty state for gallery tab", async () => {
      ...
      expect(
        screen.getByText("Your Doggo Sanctuary is empty!"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Start by fetching some adorable friends"),
      ).toBeInTheDocument();
  ...
  describe("interactions", () => {
    ...
    it("should toggle favorite", async () => {
      ...
      expect(defaultFavoritesReturn.toggleFavorite).toHaveBeenCalledWith(
        mockDogs[0],
      );
    });
  ...
  describe("keyboard shortcuts", () => {
    ...
    it("should handle f key for fetch tab", () => {
      ...
      expect(screen.getByRole("tab", { name: /Fetch Doggos/i })).toHaveClass(
    ...
    it("should handle g key for gallery tab", () => {
      ...
      expect(screen.getByRole("tab", { name: /Favorites/i })).toHaveClass(
        "doggo-sanctuary-tab active",
      );
    });
  ...
  describe("memoization", () => {
    it("should not re-render when props are the same", () => {
      const { rerender } = render(
        <DogGallery isOpen={true} onClose={jest.fn()} />,
      );
    ...
    it("should re-render when isOpen changes", () => {
      const { rerender } = render(
        <DogGallery isOpen={false} onClose={jest.fn()} />,
      );
