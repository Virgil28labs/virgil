...
jest.mock("./DogCard", () => ({
  DogCard: jest.fn(
    ({ dog, index, isFavorited, onImageClick, onFavoriteToggle }) => (
...
describe("DogGrid", () => {
  ...
  const defaultProps = {
    dogs: mockDogs,
    isFavorited: jest.fn(
      (url: string) => url === "https://example.com/dog2.jpg",
    ),
  ...
  describe("rendering", () => {
    ...
    it("should call isFavorited for each dog", () => {
      ...
      expect(defaultProps.isFavorited).toHaveBeenCalledWith(
  ...
  describe("interactions", () => {
    it("should handle image click correctly", () => {
      ...
      const firstDogCardCall = (DogCard as unknown as jest.Mock).mock
        .calls[0][0];
      ...
      expect(defaultProps.onImageClick).toHaveBeenCalledWith(
        "https://example.com/dog1.jpg",
      );
    });
    ...
    it("should handle favorite toggle correctly", () => {
      ...
      const secondDogCardCall = (DogCard as unknown as jest.Mock).mock
        .calls[1][0];
    ...
    it("should handle multiple image clicks", () => {
      ...
      mockDogs.forEach((_dog, index) => {
        const dogCardCall = (DogCard as unknown as jest.Mock).mock.calls[
          index
        ][0];
      ...
      expect(defaultProps.onImageClick).toHaveBeenNthCalledWith(
        1,
        "https://example.com/dog1.jpg",
      );
      expect(defaultProps.onImageClick).toHaveBeenNthCalledWith(
    ...
    it("should handle multiple favorite toggles", () => {
      ...
      expect(defaultProps.onFavoriteToggle).toHaveBeenNthCalledWith(
        1,
        mockDogs[0],
      );
      expect(defaultProps.onFavoriteToggle).toHaveBeenNthCalledWith(
  ...
  describe("memoization", () => {
    it("should not re-render when props are the same", () => {
      ...
      const initialCallCount = (DogCard as unknown as jest.Mock).mock.calls
        .length;
    ...
    it("should re-render when dogs array changes", () => {
      ...
      const newDogs = [
        ...mockDogs,
        { url: "https://example.com/dog4.jpg", breed: "husky", id: "dog-4" },
      ];
    ...
    it("should re-render when isFavorited function changes", () => {
      ...
      const lastThreeCalls = (DogCard as unknown as jest.Mock).mock.calls.slice(
        -3,
      );
    ...
    it("should maintain handler references when re-rendering with same callbacks", () => {
      ...
      const firstRenderHandlers = (DogCard as unknown as jest.Mock).mock
        .calls[0][0];

      // Re-render with same props
      rerender(<DogGrid {...defaultProps} />);

      const secondRenderHandlers = (DogCard as unknown as jest.Mock).mock
        .calls[0][0];

      // Handlers should be the same reference due to useCallback
      expect(firstRenderHandlers.onFavoriteToggle).toBe(
        secondRenderHandlers.onFavoriteToggle,
      );
    });
  });
