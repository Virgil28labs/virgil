...
describe("VirgilChatbot", () => {
  ...
  describe("Chatbot Dialog", () => {
    it("renders chatbot dialog with correct structure", async () => {
      ...
      const input = screen.getByRole("textbox", {
        name: /Type your message to Virgil/i,
      });
    ...
    it("displays welcome message for authenticated user", async () => {
      ...
      expect(
        screen.getByText(/good afternoon, test user/i),
      ).toBeInTheDocument();
    });
  ...
  describe("Message Sending", () => {
    it("sends message when form is submitted", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Hello! I am Virgil, your helpful assistant."),
        ).toBeInTheDocument();
      });
    });
              ...
              () =>
                resolve({
                  ...
                  json: async () => ({
                    success: true,
                    message: {
                      content: "Hello! I am Virgil, your helpful assistant.",
                    },
                  }),
                }),
              100,
  ...
  describe("Quick Actions", () => {
    it("displays quick actions when no messages", async () => {
      ...
      expect(
        screen.getByRole("button", { name: /tell me about virgil/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /how do i use this app/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /search for latest news/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /what can you do/i }),
      ).toBeInTheDocument();
    });
    ...
    it("sends message when quick action is clicked", async () => {
      ...
      const quickAction = screen.getByRole("button", {
        name: /tell me about virgil/i,
      });
  ...
  describe("Model Selection", () => {
    ...
    it("opens model dropdown when clicked", async () => {
      ...
      expect(
        screen.getByRole("option", {
          name: /GPT-4o Mini.*Fast and efficient/i,
        }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: /GPT-4o.*Most capable model/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", {
          name: /GPT-4 Turbo.*Balanced performance/i,
        }),
      ).toBeInTheDocument();
    });
    ...
    it("changes model when option is selected", async () => {
      ...
      const gpt4OptionInDropdown = gpt4Options.find(
        (option) => option.closest(".model-option") !== null,
      )!;
      await user.click(gpt4OptionInDropdown);

      expect(screen.getByText("GPT-4o")).toBeInTheDocument();
      expect(Storage.prototype.setItem).toHaveBeenCalledWith(
        "virgil-selected-model",
        "gpt-4o",
      );
    });
  });
  ...
  describe("Web Search Integration", () => {
    it("shows search indicator when searching", async () => {
      ...
      (searchService.extractSearchQuery as jest.Mock).mockReturnValue(
        "latest news",
      );
      ...
      await waitFor(() => {
        expect(
          screen.queryByText("🔍 Searching the web..."),
