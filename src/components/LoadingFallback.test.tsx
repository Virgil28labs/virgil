...
describe("LoadingFallback", () => {
  ...
  it("applies correct classes for all combinations", () => {
    ...
    combinations.forEach(({ size, variant }) => {
      ...
      if (variant === "spinner") {
        ...
        expect(screen.getAllByTestId("skeleton-loader").length).toBeGreaterThan(
          0,
        );
      }
  ...
  it("does not render message when variant is skeleton", () => {
    render(
      <LoadingFallback variant="skeleton" message="This should not appear" />,
    );
