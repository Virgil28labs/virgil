...
export const RhythmMachineViewer = memo(function RhythmMachineViewer({
  ...
  const savePattern = useCallback(
    (slotIndex: number) => {
      ...
      const category = genreInput.toLowerCase().includes("techno")
        ? "techno"
        : genreInput.toLowerCase().includes("house")
          ? "house"
          : genreInput.toLowerCase().includes("trap")
            ? "trap"
            : genreInput.toLowerCase().includes("break")
              ? "breakbeat"
              : genreInput.toLowerCase().includes("minimal")
                ? "minimal"
                : undefined;
  ...
  // Clear save slot
  const clearSaveSlot = useCallback(
    (slotIndex: number) => {
      const newSlots = [...saveSlots];
    ...
    >
      ...
      >
        ...
        <div className="rhythm-machine-header">
          <div>
            <h2 className="rhythm-machine-title">Rhythm Machine</h2>
          </div>
          <button
          ...
          <div className="rhythm-machine-sequencer">
            {DRUM_SOUNDS.map((drum, drumIndex) => (
              <div key={drum.name} className="rhythm-machine-drum-row">
                ...
                <div
                  className="rhythm-machine-steps"
                  data-steps={currentBarLength.steps}
                >
            ...
            <div className="rhythm-machine-save-buttons">
              {saveSlotIndices.map((index) => {
                ...
                return (
                  <div
                    key={index}
                    className="rhythm-machine-save-slot-container"
                  >
                    <div
                      className={`rhythm-machine-save-slot ${slot ? "occupied" : "empty"}`}
                    >
