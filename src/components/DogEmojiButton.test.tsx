  ...
  DogGallery: ({
    ...
    return (
      <div data-testid="dog-gallery" role="dialog" aria-label="Doggo Sanctuary">
        ...
        <button onClick={onClose} aria-label="Close sanctuary">
          ×
        </button>
        <div role="tablist">
...
describe("DogEmojiButton", () => {
  ...
  it("opens modal when button is clicked", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("dialog", { name: "Doggo Sanctuary" }),
      ).toBeInTheDocument();
  ...
  it("shows sanctuary content when opened", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("dialog", { name: /Doggo Sanctuary/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Your personal collection of adorable companions"),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("tab", { name: /Fetch Doggos/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("tab", { name: /My Collection/i }),
      ).toBeInTheDocument();
    });
  });
  ...
  it("uses lazy loading for DogGallery", () => {
    ...
    expect(
      screen.queryByRole("dialog", { name: "Doggo Sanctuary" }),
