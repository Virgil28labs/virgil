...
const VirgilChatbot = memo(function VirgilChatbot() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        showModelDropdown &&
        !(event.target as Element)?.closest(".model-selector")
      ) {
        setShowModelDropdown(false);
      }
      if (
        showProfileDropdown &&
        !(event.target as Element)?.closest(".profile-selector")
      ) {
...
Provide contextual help based on what the user is currently experiencing.`;
    ...
    const basePrompt =
      customSystemPrompt ||
      "You are Virgil, a helpful assistant that provides contextual help and can search the web for current information.";
    ...
    async (messageText: string) => {
      ...
      try {
        ...
        if (searchService.detectSearchIntent(messageText)) {
          ...
          try {
            ...
            if (process.env.NODE_ENV === "development") {
              console.warn(
                "Search failed, continuing with regular chat:",
                searchError,
              );
            }
          } finally {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error || `Chat service error: ${response.status}`,
          );
        }
    ...
    >
      ...
      >
        ...
        {(isSearching || isTyping) && (
          <div
            className="message assistant-msg"
            role="status"
            aria-label={
              isSearching ? "Virgil is searching" : "Virgil is typing"
            }
          >
            <div className="msg-avatar" aria-hidden="true">
              <span className="chatbot-avatar-v">V</span>
            ...
            <div className="msg-content">
              ...
              <span className="sr-only">
                {isSearching
                  ? "Virgil is searching the web"
                  : "Virgil is typing a response"}
              </span>
            </div>
          </div>
        )}
