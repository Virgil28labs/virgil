...
export const usePhotoGallery = () => {
  ...
  const updateGalleryState = useCallback(
    (updates: Partial<PhotoGalleryState>) => {
    ...
    switch (galleryState.filter) {
      ...
      case "recent":
        ...
        currentPhotos = currentPhotos.filter(
          (photo) => photo.timestamp > oneDayAgo,
        );
        break;
      case "all":
      default:
  ...
  const handlePhotoCapture = useCallback(
    async (dataUrl: string, name?: string): Promise<SavedPhoto | null> => {
      try {
  ...
  const handlePhotoDelete = useCallback(
    ...
    [
      deletePhoto,
      galleryState.selectedPhoto,
      galleryState.selectedPhotos,
      setSelectedPhoto,
      updateGalleryState,
    ],
  );
  ...
  const handleBulkFavorite = useCallback(
    async (makeFavorite: boolean): Promise<number> => {
      const photoIds = Array.from(galleryState.selectedPhotos);
      let successCount = 0;
  ...
  const navigatePhoto = useCallback(
    (direction: "next" | "previous") => {
      ...
      const currentIndex = currentPhotos.findIndex(
        (photo) => photo.id === galleryState.selectedPhoto?.id,
      );
      ...
      if (direction === "next") {
        ...
        newIndex =
          currentIndex === 0 ? currentPhotos.length - 1 : currentIndex - 1;
      }
  ...
  const getPhotoStats = useCallback(() => {
    ...
    photos.length,
    favorites.length,
    galleryState.selectedPhotos.size,
  ]);
