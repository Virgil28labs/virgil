...
export const usePhotos = () => {
  ...
  const updatePhoto = useCallback(
    async (
      ...
      try {
        ...
        if (updatedPhoto) {
          setPhotos((prev) =>
            prev.map((photo) => (photo.id === photoId ? updatedPhoto : photo)),
          );
  ...
  const getPhotoById = useCallback(
    (photoId: string): SavedPhoto | null => {
  ...
  const getStorageInfo = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to get storage info",
      );
      return null;
    }
  }, []);
  ...
  const downloadPhoto = useCallback(
    async (photo: SavedPhoto): Promise<boolean> => {
      try {
        setError(null);
