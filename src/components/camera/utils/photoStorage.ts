...
export class PhotoStorage {
  ...
  static async initialize(): Promise<void> {
    try {
      ...
      // Initialization errors are non-critical - storage will work without migration/cleanup
    }
  }
  ...
  static async getAllPhotos(): Promise<SavedPhoto[]> {
    try {
      ...
      // Return empty array if photos can't be loaded
      return [];
    }
  }
  ...
  static async getStorageInfo(): Promise<{
    ...
    try {
      ...
      // Return default values if storage info can't be retrieved
      return {
        totalPhotos: 0,
        totalSize: 0,
  ...
  static async importPhotos(jsonData: string): Promise<number> {
    try {
      ...
      const newPhotos = importData.photos.filter(
        (photo: SavedPhoto) => !existingIds.has(photo.id),
      );
  ...
  static getStorageOptions(): PhotoStorageOptions {
    try {
      ...
      // Return default options if settings can't be loaded
  ...
  private static async cleanupOldPhotos(): Promise<void> {
    ...
    try {
      const photos = await this.getAllPhotos();
      const cutoffTime =
        Date.now() - options.cleanupAfterDays * 24 * 60 * 60 * 1000;

      const photosToKeep = photos.filter(
        (photo) => photo.timestamp > cutoffTime || photo.isFavorite,
      );

      if (photosToKeep.length < photos.length) {
        await this.savePhotos(photosToKeep);
        // Successfully cleaned up old photos
      }
    } catch (error) {
