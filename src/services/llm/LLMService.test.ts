...
describe("LLMService", () => {
  ...
  describe("complete", () => {
    ...
    it("uses cache when enabled and cache key provided", async () => {
      ...
      expect(global.fetch).not.toHaveBeenCalled();
      expect(service.emit).toHaveBeenCalledWith("cache-hit", {
        cacheKey: "test-key",
      });
    });
  ...
  describe("completeStream", () => {
    ...
    it("handles stream errors", async () => {
      ...
      expect(service.emit).toHaveBeenCalledWith("stream-error", {
        error: "Stream error",
      });
    });
  });
  ...
  describe("getModels", () => {
    it("fetches available models", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        "http://localhost:5002/api/v1/llm/models",
      );
    });
    ...
    it("returns empty object on error", async () => {
      ...
      expect(service.emit).toHaveBeenCalledWith("error", {
        error: "Fetch error",
      });
    });
  });
  ...
  describe("countTokens", () => {
    ...
    it("falls back to estimation on error", async () => {
      ...
      const result = await service.countTokens(
        "Test text for token estimation",
      );

      // Fallback estimation: text.length / 4
      expect(result).toBe(
        Math.ceil("Test text for token estimation".length / 4),
      );
    });
  });
